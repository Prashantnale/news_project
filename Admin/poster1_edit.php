              ...
              <form method="post" enctype="multipart/form-data">
                ...
                if(isset($_POST['submitd']))
                {
                  ...
                  if($title!="" && $cat!=""  && $dis!=""  && $timed!=""  && $folder!="" && $idd!="")
                  {
                    if(!$timg)
                    {
                  $quuery="UPDATE `poster1` SET `p_title`='$title',`p_description`='$dis',`p_date`='$timed',`p_category`='$cat' WHERE `id`='$idd'";
                    }
                    else{
                      $quuery="UPDATE `poster1` SET `p_title`='$title',`p_description`='$dis',`p_date`='$timed',`p_category`='$cat',`p_img`='$folder' WHERE `id`='$idd'";
                    }
                  $data=mysqli_query($conn,$quuery);
                  if($data)
                  {
                    echo "<script>location.href='poster1.php';</script>";
                  }
                  else
                  {
